...
class WCIBConnection:
    ...
    def _upload_extra(
        ...
        body["data"] = open(fname, 'rb').read()

        j = {}

        if not dryrun:
            response = self._s.put(
                pth, headers=headers, data=body, timeout=self.timeout
            )
            response.raise_for_status()
            j = response.json()
    ...
    def _upload_data(
        ...
        if not dryrun:
            ...
            # payload = (('data', open(fname, 'rb')), ('size': ), ('filename', os.path.basename(fname)))
            ...
            #     response = self._s.put(
            #         pth, headers=headers, data=files, timeout=self.timeout
            #     )
