...
in the upload:
```sh
./main.py -U 17 -s "./dataset/file*.tgz" -t user.token --tag radio --tag drive-test
```

Annotate an existing file without re-uploading, using `--setmeta <FileID>`
(the dataset id comes from `-U`/`--upload` or `-l`/`--listfiles`):
```sh
./main.py --setmeta 74 -U 17 -t user.token --tag reviewed
```

A POI is given as `--poi 'start,stop,text'`. `start`/`stop` are timestamps
(ISO-8601 or epoch, no commas); the text may contain commas. POIs only apply to
a **single** file — either a single-file upload or `--setmeta`:
```sh
./main.py --setmeta 74 -U 17 -t user.token \
